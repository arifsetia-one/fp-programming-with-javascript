...
function findTicketAvailability(movie, user) {
    return user.favoriteGenre.includes(movie.genre) && movie.capacity - movie.soldTicket >= user.ticket;
}

function findRecommendation(user) {
    return findMovies(user.favoriteGenre).filter((movie) => findTicketAvailability(movie, user));
}
...
function generateRecommendation(user) {
    ...
    const recommendation = findRecommendation(user).map((recommendation) => {
        const { id, name, genre } = recommendation;
        return { id, name, genre, totalPrice: user.ticket * prices[genre] };
    });
